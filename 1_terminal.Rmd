...
rm # remove files - there is no trash!
...
<div class = "note">
The `~` returns your home directory. This is generally your user directory by default.
</div>
...
<div class = "note">
All command line applications include `flags`. These provide access to additional options. They are pre-pended with a `-`.
</div>
...
```bash
mkdir 2022_DS-Workshop
```

<div class = "note">
Once you create this folder, open it beside your console window and you\'ll be able to see it updated as you work.
</div>
...
* When dates are used, they should be in the format **yyyymmdd**. They should contain no special characters, this includes spaces. Parts of file name--dates, affiliated project, description--should be joined with underscores `_`, while multiple terms within a part can be in `CamelBack` or separated with dashes `-`.
* One exception to this general guidance is that if your document is destined to live on the web, so that it gets indexed properly, you should also avoid underscores `_`.
* A second exception to this general guidance is that if you\'re going to move files between Macs, Linux, and Windows, consider using only lower case characters.
</div>

File naming diatribe done, rename the file we just created to give it a meaningful name

```bash
mv myNewFile.md 20220427_DS-Workshop_Web-Materials.md
```
...
### Retrieving remote files 2

In the next section, we\'ll be working with a Markdown template, so let's download that template.

url: `https://vdunbar.github.io/ds-introduction/files/academic-cv_template.md`

```bash
curl https://vdunbar.github.io/ds-introduction/files/academic-cv_template.md -o 20220427_MD-Template_MVD.md
```

<div class = "note">
We use the `o`--for output--flag to name the file to put the download into.
</div>

We\'ll also be working with at least one image, so we\'ll make a directory to hold images and then download an image

url: `https://raw.githubusercontent.com/vdunbar/ds-introduction/main/images/twitter.png`

```bash
mkdir images

curl https://raw.githubusercontent.com/vdunbar/ds-introduction/main/images/twitter.png -o images/twitter.png
```
...
<div class = "note">
The `*` is a wildcard and tells `mv`, move anything that ends in `.tmp`
</div>
...
# stop doing things
for i in *.tmp; do mv $i 20220427_DS-Workshop_$i; done
```
...
<div class = "note">
Every folder has in it two place holders, one labeled `.` and the other labeled `..`. `.` refers to the current directory, while `..` refers to the directory one level up in the hierarchy. To see these listed, you can call `ls` with the `a` flag.
</div>

Next, [we look at Markdown...](2_markdown.html)
